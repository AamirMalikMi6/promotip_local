...
class CreditController extends AbstractController
{
    ...
    /**
     * @Route({
     *     "en": "/dashboard/credit/store",
     *     "nl": "/dashboard/credit/store",
     *     "fr": "/dashboard/credit/store"
     * }, name="dashboard_store_credit")
     */
    ...
    /**
     * @Route({
     *     "en": "/dashboard/user/mollie/add_customer_info",
     *     "nl": "/dashboard/user/mollie/add_customer_info",
     *     "fr": "/dashboard/user/mollie/add_customer_info"
     * }, name="dashboard_user_add_mollie_info")
     */
    public function userAddMollieCustomerInfo(Request $request,
                                              UserInterface $user,
    ...
    )
    {
        ...
        if ( $payments->count > 0 )
        {
            ...
            if ($lastPayment->isPaid() && !$lastPayment->hasRefunds() && !$lastPayment->hasChargebacks())
            {
                ...
                $invoice = new Invoice();
                // $invoice->setPrefix($this->getParameter('invoice_prefix'));
                ...
                $vatAmount = 0;
                if ( $vatPercent > 0 ) $vatAmount = ($lastPayment->amount->value * $vatPercent) / 100;
