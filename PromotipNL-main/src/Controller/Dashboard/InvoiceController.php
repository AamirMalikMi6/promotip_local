...
class InvoiceController extends AbstractController
{
     ...
     *
     * @Route({
     *     "en": "/dashboard/facturen",
     *     "nl": "/dashboard/facturen",
     *     "fr": "/dashboard/facturen"
     * }, name="dashboard_facturen")
     */
    ...
    )
    {
        ...
        if (in_array('ROLE_ROOT', $user->getRoles())) {
            $builder->add('user', TextType::class, [
                ...
                'data' => $request->query->get('form')['user'] ?? ''
            ]);
        }
...
//        $qb->setParameter('userId', $user->getId());
        ...
        if($request->query->has('form') && trim($request->query->get('form')['startDate'] ?? '') !== ''){
            $qb->andWhere('DATE(invoice.invoiceDate) >= :startDate')->setParameter('startDate', $request->query->get('form')['startDate']);
        }

        if($request->query->has('form') && trim($request->query->get('form')['endDate'] ?? '') !== ''){
            $qb->andWhere('DATE(invoice.invoiceDate) <= :endDate')->setParameter('endDate', $request->query->get('form')['endDate']);
        }
        ...
        if (!in_array('ROLE_ROOT', $user->getRoles())) {
            ...
            if ($request->query->has('form') && trim($request->query->get('form')['user'] ?? '') !== '') {
                $qb->join(User::class, 'user', Join::WITH, 'user.id = invoice.userId');
                $qb->andWhere('user.id = :user OR user.email = :user');
                $qb->setParameter('user', $request->query->get('form')['user']);
            }
        }
        ...
        return $this->render(
            'dashboard/facturen/index.html.twig',
            [
                'balance' => $user->getCredits(),
                'user' => $user,
                'invoices' => $invoices,
     ...
     *
     * @Route({
     *     "en": "/dashboard/facturen/{invoice}",
     *     "nl": "/dashboard/facturen/{invoice}",
     *     "fr": "/dashboard/facturen/{invoice}"
     * }, name="dashboard_facturen_download")
     */
